...
import { Badge, BronzeBadge, ColourBadge, GoldBadge, SilverBadge } from "./Badges";
    ...
    <section className="container overflow-x-auto rounded-lg mb-5">
      <table className="table-auto w-full">
        <thead>
          <tr className="text-md font-semibold tracking-wide text-left bg-zinc-50 text-zinc-500 uppercase">
            <th className="border-0 px-4 py-3">
              <input type="checkbox" checked={props.isAllChecked} onChange={() => props.onToggleAll()} />
            </th>
        ...
        <tbody className="bg-white">
          {props.data.map((row) => (
            <tr className="text-gray-700 hover:bg-zinc-100" onClick={() => props.onRowClick(row.Person)} key={row.Person}>
              <td>
                <span className="px-2">
                  <ColourBadge hex={GetColourForName(row.Person, props.names)} title={row.Person} isChecked={row.isChecked} />
                </span>
              </td>
              ...
              <td className="border-0 px-4 py-3 font-semibold">
                {row.SRank > 3 && OrderSuffix(row.SRank)}
              ...
              <td className="border-0 px-4 py-3">
                ...
                {row.Bonus !== 0 && (
                  <>
                    <Badge className="bg-gray-400 text-white px-2" title="Bonus">
                      {row.Bonus}
                    </Badge>
                  </>
                )}
